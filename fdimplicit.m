...
for i = 1:1:sn %iterate at each grid point
    a(i) = 0.5*dt*(r*i - sig2*i^(2*g)*ds^(2*g-2));
    b(i) = 1 + dt*(sig2*i^(2*g)*ds^(2*g-2) + r);
    c(i) = -0.5*dt*(r*i + sig2*i^(2*g)*ds^(2*g-2));

end
C = diag(a(3:end-1),-1) + diag(b(2:end-1)) + diag(c(2:end-2),1);
...
for n = tn:-1:2
    ...
    f(end) = f(end) - c(end-1)*vj(end); % for the v(t(i-1), s(n-1))
    v(2:end-1) = C\f;
    vj = v;
end
end
